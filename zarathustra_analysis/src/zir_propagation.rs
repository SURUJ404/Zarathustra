...
use std::fmt;
use std::ops::*;
use zarathustra_ast::common::{ResultFold, WithSpan};
use zarathustra_ast::zir::types::UBitwidth;
use zarathustra_ast::zir::AssertionStatement;
use zarathustra_ast::zir::IfElseStatement;
use zarathustra_ast::zir::{
...
impl<'ast, T: Field> ResultFolder<'ast, T> for ZirPropagator<'ast, T> {
    ...
    fn fold_assembly_block(
        &mut self,
        s: zarathustra_ast::zir::AssemblyBlockStatement<'ast, T>,
        ...
        Ok(match block.is_empty() {
            ...
            false => vec![ZirStatement::Assembly(
                zarathustra_ast::zir::AssemblyBlockStatement::new(block),
            )],
        })
    }
    ...
    fn fold_assembly_assignment(
        &mut self,
        s: zarathustra_ast::zir::AssemblyAssignment<'ast, T>,
    ...
    fn fold_assembly_constraint(
        &mut self,
        s: zarathustra_ast::zir::AssemblyConstraint<'ast, T>,
    ) -> Result<Vec<ZirAssemblyStatement<'ast, T>>, Self::Error> {
    ...
    fn fold_definition_statement(
        &mut self,
        s: zarathustra_ast::zir::DefinitionStatement<'ast, T>,
    ) -> Result<Vec<ZirStatement<'ast, T>>, Self::Error> {
    ...
    fn fold_if_else_statement(
        &mut self,
        s: zarathustra_ast::zir::IfElseStatement<'ast, T>,
    ) -> Result<Vec<ZirStatement<'ast, T>>, Self::Error> {
        {
    ...
    fn fold_multiple_definition_statement(
        &mut self,
        s: zarathustra_ast::zir::MultipleDefinitionStatement<'ast, T>,
    ) -> Result<Vec<ZirStatement<'ast, T>>, Self::Error> {
    ...
    fn fold_select_expression<
        E: Clone + Expr<'ast, T> + ResultFold<Self, Self::Error> + zarathustra_ast::zir::Select<'ast, T>,
    >(
        &mut self,
        _: &E::Ty,
        e: SelectExpression<'ast, T, E>,
    ) -> Result<zarathustra_ast::zir::SelectOrExpression<'ast, T, E>, Self::Error> {
...
mod tests {
    use super::*;
    use zarathustra_ast::zir::{Id, RuntimeError};
    ...
    mod field {
        ...
        use zarathustra_ast::zir::Select;
