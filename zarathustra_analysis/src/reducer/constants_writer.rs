    ...
    fn fold_constant_symbol_declaration(
        ...
        match d.symbol {
            TypedConstantSymbol::Here(c) => {
                ...
                use zarathustra_ast::typed::folder::Folder;

                let c = ConstantsReader::with_constants(&self.constants).fold_constant(c);

                use zarathustra_ast::typed::{DeclarationSignature, TypedFunction, TypedStatement};
                ...
                if let TypedStatement::Return(ret) = inlined_wrapper.statements.pop().unwrap() {
                    ...
                    if zarathustra_ast::typed::types::try_from_g_type::<_, UExpression<'ast, T>>(
                        c.ty.clone(),
                    )
                    .unwrap()
                        == expression.get_type()
