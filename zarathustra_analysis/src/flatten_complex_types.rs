...
use std::ops::*;
use zarathustra_ast::common::expressions::{BinaryExpression, ValueExpression};
use zarathustra_ast::common::operators::OpEq;
use zarathustra_ast::common::statements::LogStatement;
use zarathustra_ast::common::{Span, WithSpan};
use zarathustra_ast::typed::types::{ConcreteArrayType, IntoType, UBitwidth};
use zarathustra_ast::typed::{self, Basic, Expr, Typed};
use zarathustra_ast::zir::{self, Expr as ZirExpr, Folder, Id, MultipleDefinitionStatement, Select};
use zarathustra_ast::zir::{IntoType as ZirIntoType, SourceIdentifier};
...
impl<'ast, T: Field> Flattener<T> {
    ...
    fn fold_declaration_parameter(
        ...
        self.fold_variable(zarathustra_ast::typed::variable::try_from_g_variable(p.id).unwrap())
            .into_iter()
...
fn fold_conditional_expression<'ast, T: Field, E: Flatten<'ast, T>>(
    ...
    use zarathustra_ast::zir::Conditional;
...
fn fold_eq_expression<'ast, T: Field, E: Flatten<'ast, T>>(
    f: &mut Flattener<T>,
    statements_buffer: &mut Vec<zir::ZirStatement<'ast, T>>,
    e: zarathustra_ast::common::expressions::BinaryExpression<
        OpEq,
        E,
        E,
...
fn fold_function<'ast, T: Field>(
    ...
    zir::ZirFunction {
        ...
        signature: typed::types::ConcreteSignature::try_from(
            zarathustra_ast::typed::types::try_from_g_signature::<
                zarathustra_ast::typed::types::DeclarationConstant<'ast, T>,
                zarathustra_ast::typed::UExpression<'ast, T>,
            >(fun.signature)
            .unwrap(),
        )
        .unwrap()
