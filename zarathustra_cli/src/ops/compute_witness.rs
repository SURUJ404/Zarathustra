...
use zarathustra_ast::ir::{self, ProgEnum};
use zarathustra_ast::typed::{
    abi::Abi,
    types::{ConcreteSignature, ConcreteType, GTupleType},
};
use zarathustra_circom::write_witness;
...
pub fn subcommand() -> App<'static, 'static> {
    SubCommand::with_name("compute-witness")
        ...
        .long("abi")
        .help("Use ABI encoding. Arguments are expected as a JSON object as specified at zarathustra.github.io/toolbox/abi.html#abi-input-format")
...
fn cli_compute<'a, T: Field, I: Iterator<Item = ir::Statement<'a, T>>>(
        ...
        true => {
            ...
            match is_abi {
                true => match stdin.read_to_string(&mut input) {
                    Ok(_) => {
                        use zarathustra_abi::parse_strict;
    ...
    let results_json_value: serde_json::Value =
        zarathustra_abi::Value::decode(witness.return_values(), *signature.output).into_serde_json();
