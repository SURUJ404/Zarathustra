...
use std::fmt;
use zarathustra_abi::{Decode, Value};
use zarathustra_ast::ir::{
    LinComb, Parameter, QuadComb, RuntimeError, Solver, Statement, Variable, Witness,
};
use zarathustra_ast::zir::{self, Expr};
...
impl Interpreter {
    ...
    pub fn execute_solver<'ast, T: Field>(
        ...
        let res = match solver {
            Solver::Zir(func) => {
                use zarathustra_ast::zir::result_folder::ResultFolder;
                ...
                let constants = func
                    ...
                    .map(|(a, v)| match &a.id.ty {
                        zir::Type::FieldElement => Ok((
                            a.id.id.clone(),
                            zarathustra_ast::zir::FieldElementExpression::value(*v).into(),
                        )),
                        zir::Type::Boolean => match v {
                            v if *v == T::from(0) => Ok((
                                a.id.id.clone(),
                                zarathustra_ast::zir::BooleanExpression::value(false).into(),
                            )),
                            v if *v == T::from(1) => Ok((
                                a.id.id.clone(),
                                zarathustra_ast::zir::BooleanExpression::value(true).into(),
                            )),
                        ...
                        zir::Type::Uint(bitwidth) => match v.bits() <= bitwidth.to_usize() as u32 {
                            true => Ok((
                                a.id.id.clone(),
                                zarathustra_ast::zir::UExpression::value(
                ...
                let mut propagator = zarathustra_analysis::ZirPropagator::with_constants(constants);
                ...
                if let zarathustra_ast::zir::ZirStatement::Return(v) =
                    folded_function.statements[0].clone()
                {
                    v.inner
                        .into_iter()
                        .map(|v| match v {
                            zarathustra_ast::zir::ZirExpression::FieldElement(
                                zarathustra_ast::zir::FieldElementExpression::Value(n),
                            ) => n.value,
                            _ => unreachable!(),
                        })
                        .collect()
            ...
            Solver::Sha256Round => {
                ...
                use zarathustra_embed::bellman::generate_sha256_round_witness;
            ...
            Solver::SnarkVerifyBls12377(n) => {
                use zarathustra_embed::ark::generate_verify_witness;
                use zarathustra_field::Bw6_761Field;
            ...
            Error::UnsatisfiedConstraint { ref error } => {
                ...
                match error {
                    Some(e) if e.is_malicious() => {
                        writeln!(f)?;
                        write!(f, "The default Zarathustra interpreter should not yield this error. Please open an issue.")
                    }
                    _ => write!(f, ""),
                }
            }
...
mod tests {
    ...
    fn solver_ref() {
        ...
        use zarathustra_ast::common::RefCall;
