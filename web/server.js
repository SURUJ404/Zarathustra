...
app.get('/api/health', (req, res) => {
    const bin = fs.existsSync(path.join(BIN_PATH, 'astra')) ? path.join(BIN_PATH, 'astra') : null;
    res.json({
        status: 'ok',
...
function runAstra(args) {
    const bin = path.join(BIN_PATH, 'astra');
    if (!fs.existsSync(bin)) {
        return { ok: false, stdout: '', stderr: 'astra binary not found at ' + bin };
    }
    log('debug', 'running', { bin, args, cwd: WORK_DIR });
    try {
        const stdout = execSync(`"${bin}" ${args.join(' ')}`, {
            cwd: WORK_DIR,
            env: { ...process.env, ASTRA_STDLIB: STDLIB_PATH },
            encoding: 'utf-8',
...
app.post('/api/compile', (req, res) => {
    ...
    const inputFile = path.join(WORK_DIR, `${id}.zara`);
    try {
        fs.writeFileSync(inputFile, code);
        const args = ['prove', 'compile', '-i', inputFile, '--curve', curve || 'bn128'];
        const result = runAstra(args);
...
app.post('/api/compute-witness', (req, res) => {
    ...
    const bin = path.join(BIN_PATH, 'astra');
    const args = ['prove', 'witness', '-i', outBin, '--output', path.join(WORK_DIR, 'witness')];
...
app.post('/api/setup', (req, res) => {
    ...
    const result = runAstra(['prove', 'setup', '-i', outBin]);
...
app.post('/api/prove', (req, res) => {
    ...
    const result = runAstra(['prove', 'generate', '-i', outBin, '-w', witnessFile, '-p', provingKey]);
...
app.post('/api/verify', (req, res) => {
    ...
    const result = runAstra(['prove', 'verify', '-v', verificationKey, '-j', proofFile]);
    res.json(result);
});
...
app.post('/api/export-verifier', (req, res) => {
    ...
    const result = runAstra(['deploy', 'verifier', '-i', verificationKey]);
    res.json(result);
});
...
if (!IS_VERCEL) {
    const server = app.listen(PORT, HOST, () => {
        ...
        console.log(`  Binary: ${BIN_PATH}/astra`);
