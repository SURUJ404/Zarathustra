    ...
    let matches = App::new("astra")
        ...
        .subcommand(SubCommand::with_name("prove").about("zkSNARK proof generation pipeline")
            .subcommand(SubCommand::with_name("compile").about("Compile a circuit source file into R1CS")
                ...
                .arg(clap::Arg::with_name("public").short("p").long("public").takes_value(true).default_value("").help("Public inputs (comma-separated)"))
                .arg(clap::Arg::with_name("private").short("r").long("private").takes_value(true).default_value("").help("Private inputs (comma-separated)")))
