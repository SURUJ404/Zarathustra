...
In this tutorial you learn how to use Zarathustra and zero knowledge proofs to reveal a single bit from the preimage of a hash value.
...
```
{{#include ../../../zarathustra_cli/examples/book/rng_tutorial/test.sh:10}}
```

The input to the Zarathustra program is sixteen 32 bit values, each in decimal. specify those values 
to get a hash. For example, to calculate the hash of `0x00000000000000010000000200000003000000040000000500000006...`
use this command:
```
{{#include ../../../zarathustra_cli/examples/book/rng_tutorial/test.sh:11}}
```
...
```
{{#include ../../../zarathustra_cli/examples/book/rng_tutorial/test.sh:27:28}}
```
...
A Zarathustra function can return multiple values. In this case, it returns the hash and a boolean which is the 
...
copies of `<value>`. It is necessary to include it here because a Zarathustra variable must be initialized
when it is declared.
...
```
{{#include ../../../zarathustra_cli/examples/book/rng_tutorial/test.sh:28:29}}
```
...
```
{{#include ../../../zarathustra_cli/examples/book/rng_tutorial/test.sh:35}}
```
...
```
{{#include ../../../zarathustra_cli/examples/book/rng_tutorial/test.sh:36}}
```
...
At this point you should know how to use Zarathustra to create zero knowledge proofs and verify them from the command
