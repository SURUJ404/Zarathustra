...
Zarathustra comes with a number of reusable components in the form of a Standard Library. In order to import it as described in the [imports](../language/imports.md) section, the `$ZARATHUSTRA_STDLIB` environment variable must be set to the `stdlib` folder.

The full Zarathustra Standard Library can be found [here](https://github.com/Zarathustra/Zarathustra/tree/latest/zarathustra_stdlib/stdlib).
