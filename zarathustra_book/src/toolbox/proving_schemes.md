...
Proving schemes supported by Zarathustra require a pairing-friendly elliptic curve. The options are the following:
...
Zarathustra supports different proving schemes. We identify the schemes by the reference to the paper that introduced them. Currently the options available are:
...
Zarathustra supports multiple backends. The options are the following:
