...
use zarathustra_ast::common::expressions::ValueExpression;
use zarathustra_ast::common::{FormatString, ModuleMap, SourceMetadata, SourceSpan, WithSpan};
use zarathustra_ast::typed::types::{GGenericsAssignment, GTupleType, GenericsAssignment};
use zarathustra_ast::typed::SourceIdentifier;
use zarathustra_ast::typed::*;
use zarathustra_ast::typed::{DeclarationParameter, DeclarationVariable, Variable};
use zarathustra_ast::untyped::Identifier;
use zarathustra_ast::untyped::*;
use zarathustra_field::Field;

use std::ops::*;

use zarathustra_ast::untyped::types::{UnresolvedSignature, UnresolvedType, UserTypeId};

use std::hash::Hash;
use zarathustra_ast::typed::types::{
...
impl<'ast, T: Field> Checker<'ast, T> {
                                ...
                                match symbol_unifier.insert_constant(declaration.id) {
                                    ...
                                    true => {
                                        ...
                                        let info = IdentifierInfo {
                                            ...
                                            ty: zarathustra_ast::typed::types::try_from_g_type(ty.clone()).unwrap(),
                                            is_mutable: false,
                                        };
                                        assert_eq!(self.scope.level, 0);
                                        assert!(!self.scope.insert(id, info));
    ...
    fn check_variable(
        &mut self,
        v: zarathustra_ast::untyped::VariableNode<'ast>,
        module_id: &ModuleId,
        types: &TypeMap<'ast, T>,
    ) -> Result<Variable<'ast, T>, Vec<ErrorInner>> {
        let ty = self
    ...
    fn check_for_loop(
        &mut self,
        var: zarathustra_ast::untyped::VariableNode<'ast>,
                    ...
                    .map(|e| {
                        match e.value {
                            Expression::FunctionCall(fun_id_expression, generics, arguments) => {
                                let ty = zarathustra_ast::typed::types::try_from_g_type(
                                    return_type.clone(),
                                )
                                .map(Some)
                                .unwrap();
        ...
        match expr.value {
            ...
            Expression::Conditional(conditional) => {
                ...
                let kind = match conditional.kind {
                    zarathustra_ast::untyped::ConditionalKind::IfElse => {
                        zarathustra_ast::typed::ConditionalKind::IfElse
                    }
                    zarathustra_ast::untyped::ConditionalKind::Ternary => {
                        zarathustra_ast::typed::ConditionalKind::Ternary
                    }
                };
...
mod tests {
    ...
    fn duplicate_main_function() {
        ...
        let main1_arguments = vec![zarathustra_ast::untyped::Parameter::public(
            untyped::Variable::immutable("a", UnresolvedType::FieldElement.mock()).mock(),
        )
        .mock()];
    ...
    mod structs {
        use super::*;
        use zarathustra_ast::typed::types::StructMember;
