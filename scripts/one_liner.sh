...
Options:
    ...
    --to LOCATION   Where to install (default ~/.zarathustra)
EOF
}
...
main() {
    ...
    if [ -z $dest ]; then
        dest="$HOME/.zarathustra"
    fi

    say_err "Installing to: $dest"

    # Fetch archive
    url="$url/download/$tag/zarathustra-$tag-$arch.$ext"
    ...
    if [ -d $dest ]; then
      if [ $force = true ]; then
        ...
        read -p "Zarathustra is already installed, overwrite (y/n)? " answer < /dev/tty
        case ${answer} in
            y|Y )
                rm -rf $dest/*
                cp -r $td/* $dest
    ...
    mkdir -p $dest/bin
    mv $dest/zarathustra* $dest/bin && chmod 755 $dest/bin/*
    rm -rf $td
...
Zarathustra was installed successfully!
If this is the first time you're installing Zarathustra run the following:
